...
class CommandeHTTP:
    ...
    ressources = { 
        re.compile( 
            "\/page\/?$" 
        ) : "./page.html" # l'outil de conception des requêtes 
    } 
    ...
    async def repondre(self, codeHTTP=200, messageHTTP="OK", entetes={}, fichier=None, corpsType="text/html"): 
        ...
        if corpsType is not None: 
            await self.repondreLigne( 
                "content-type: %s; charset=UTF-8" % corpsType 
            ) 
        if tailleReponse<0: 
            await self.repondreLigne( 
    ...
    async def cmd_get(self): 
        self.client.silo.requetes.append( 
            self 
        ) 
        try: 
    ...
    async def cmd_delete(self): 
        self.client.silo.transactions.append( 
            ( "suppression", self ) 
        ) 
        try: 
            self.filtre = FiltreHTTP( self.entetes ) 
